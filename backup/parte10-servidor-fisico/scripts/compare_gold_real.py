...
def resumir(filas: list, lote: str, prefijo: str, n_gold: int) -> dict:
    ...
    for campo in CAMPOS:
        ...
        exacto_pct = round(100 * n_exacto / n, 1)
        # conf_real es el promedio de la celda: similitud en campos no estrictos,
        # 0/100 en cédula, teléfonos, fechas y casillas.
        real = round(suma_real[campo] / n, 1)
        campos.append(
            {
                "etiqueta": campo,
                "n": n,
                ...
                "malo": round(100 * (n - n_exacto - casi) / n, 1),
                "conf_declarada": dec,
                "conf_real": real,
    ...
    real = round(sum(f["conf_real"] for f in filas) / n, 1) if n else None
    exacto_pct = round(100 * exactos / n, 1) if n else None
